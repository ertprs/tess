...
client.on("qr", qr => {
	...
	console.log(col(time),col("Scan Secepatnya Bro!", "kuning"));
});
...
const botTol2 = () => {
    ...
    if (msg.body.startsWith('!subject ')) {
        if (chat.isGroup) {
            if (dariGC.replace('@c.us', '') == chat.owner.user || dariGC.replace('@c.us','') == '6285770269605') {
	...
	text += "╰───「 Success 」"
        ...
        if (chat.isGroup) {
            if (dariGC.replace('@c.us', '') == chat.owner.user || dariGC.replace('@c.us','') == '6285770269605' || dariGC.replace('@c.us','') == '19197694653') {
...
*!igp* : Mendownload media foto dari instagram
contoh : !igp url
